...
namespace SQL_MusicApp
{
    ...
    {
        string connectionString = "datasource=127.0.0.1;port=3306;username=root;password=159875321;database=music";
        ...
        public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            // fetch items from database
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT track.track_id as trackID, title, number, video_url, lyrics, albums_ID FROM track JOIN albums ON albums_ID = albums.ID WHERE albums.ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
        ...
        internal int AddOneTrack(Track track)
        {
            ...
            // define the sql statement to add Track
            MySqlCommand command = new MySqlCommand("INSERT INTO `track` (`title`, `number`, `video_url`, `lyrics`, `albums_ID`) VALUES (@name, @number, @vidioURL,  @lyrics, @IDalboms)", connection);

            command.Parameters.AddWithValue("@name", track.Name);
            command.Parameters.AddWithValue("@number", track.Number);
            command.Parameters.AddWithValue("@vidioURL", track.Video_URL);
            command.Parameters.AddWithValue("@lyrics", track.Lyrics);
            command.Parameters.AddWithValue("@IDalboms", track.IDalboms);
